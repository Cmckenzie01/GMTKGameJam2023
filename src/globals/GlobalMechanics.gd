...
var BulletScene = preload('res://src/towers/bullet.tscn')
var CaltropScene = preload('res://src/towers/caltrop.tscn')
...
func spawn_caltrop(spawner_position: Vector2, destination: Vector2, lifespan: int, damage: int, container: Node2D) -> void:
    ...
    caltrop.apply_central_impulse(Vector2(destination.x, 0))
    
    print(caltrop.global_position)
    
    # caltrop.trajectory = (destination - spawner_position).normalized()
