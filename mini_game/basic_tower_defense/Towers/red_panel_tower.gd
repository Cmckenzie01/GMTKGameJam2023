...
@onready var tower = preload("res://mini_game/basic_tower_defense/Towers/red_bullet_tower.tscn")
var currTile
...
func _on_gui_input(event):
	if Game.Gold >= 10:
		...
		if event is InputEventMouseButton and event.button_mask == 1:
			...
			get_child(1).global_position= (event.global_position * 3)
			...
			if get_child_count() > 1:
				...
				if currTile == Vector2i(4, 5):
					...
					else: get_child(1).get_node("Area").modulate = Color(0, 255, 0, 0.3)
				else:
					get_child(1).get_node("Area").modulate = Color(255, 0, 0)
			...
			if event.global_position.x * 3 >= 2944:
				...
				if currTile == Vector2i(4, 5):
					...
					if (targets.size() < 1):
						...
						tempTower.global_position = (event.global_position * 3)
